...
func TestDeleteNFT(t *testing.T) {
	...
	err := NFTKeeper.DeleteNFT(ctx, Denom1, ID1, sdk.NewInt(1))
	require.Error(t, err)
	...
	err = NFTKeeper.DeleteNFT(ctx, Denom1, ID2, sdk.NewInt(1))
	require.Error(t, err)

	// DeleteNFT should fail when delete quantity is more than the reserved one
	err = NFTKeeper.DeleteNFT(ctx, Denom1, ID1, sdk.NewInt(4))
	require.Error(t, err)
