...
func querySupply(ctx sdk.Context, path []string, req abci.RequestQuery, k Keeper) ([]byte, error) {
	var params types2.QueryCollectionParams
	...
	if !found {
		return nil, sdkerrors.Wrap(types2.ErrUnknownCollection, fmt.Sprintf("unknown denom %s", params.Denom))
	}

	bz, err := types2.ModuleCdc.MarshalJSON(strconv.Itoa(collection.Supply()))
	if err != nil {
...
func queryOwner(ctx sdk.Context, path []string, req abci.RequestQuery, k Keeper) ([]byte, error) {
	var params types2.QueryBalanceParams
	...
	bz, err := types2.ModuleCdc.MarshalJSON(owner)
	if err != nil {
		return nil, sdkerrors.Wrap(sdkerrors.ErrJSONMarshal, err.Error())
	}
...
func queryCollection(ctx sdk.Context, path []string, req abci.RequestQuery, k Keeper) ([]byte, error) {
	...
	collections := types2.NewCollections(collection)
	bz, err := types2.ModuleCdc.MarshalJSON(collections)
	if err != nil {
		return nil, sdkerrors.Wrap(sdkerrors.ErrJSONMarshal, err.Error())
	}
...
func queryDenoms(ctx sdk.Context, path []string, req abci.RequestQuery, k Keeper) ([]byte, error) {
	...
	bz, err := types2.ModuleCdc.MarshalJSON(denoms)
	if err != nil {
		return nil, sdkerrors.Wrap(sdkerrors.ErrJSONMarshal, err.Error())
	}
...
func queryNFT(ctx sdk.Context, path []string, req abci.RequestQuery, k Keeper) ([]byte, error) {
	var params types2.QueryNFTParams
	...
	if err != nil {
		return nil, sdkerrors.Wrap(types2.ErrUnknownNFT, fmt.Sprintf("invalid NFT #%s from collection %s", params.TokenID, params.Denom))
	}

	bz, err := types2.ModuleCdc.MarshalJSON(nft)
	if err != nil {
		return nil, sdkerrors.Wrap(sdkerrors.ErrJSONMarshal, err.Error())
	}
