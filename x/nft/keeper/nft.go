...
func (k Keeper) GetNFT(ctx sdk.Context, denom, id string) (exported.NFT, error) {
	...
	if !found {
		return nil, sdkerrors.Wrap(types2.ErrUnknownCollection, fmt.Sprintf("collection of %s doesn't exist", denom))
	}
...
func (k Keeper) MintNFT(ctx sdk.Context, denom string, nft exported.NFT) error {
	...
	if found {
		...
		collection = types2.NewCollection(denom, types2.NewNFTs(nft))
	}
...
func (k Keeper) DeleteNFT(ctx sdk.Context, denom, id string, quantity sdk.Int) error {
	collection, found := k.GetCollection(ctx, denom)
	if !found {
		return sdkerrors.Wrap(types2.ErrUnknownCollection, fmt.Sprintf("collection of %s doesn't exist", denom))
	}
	nft, err := collection.GetNFT(id)
	if err != nil {
		return err
	...
	if quantity.GT(nft.GetOwners().GetOwner(nft.GetCreator()).GetQuantity()) {
		return sdkerrors.Wrap(types2.ErrNotAllowedBurn,
