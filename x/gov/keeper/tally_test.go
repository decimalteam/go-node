...
func TestTallyOnlyValidatorsAllYes(t *testing.T) {
	...
	require.NoError(t, err)
	proposalID := proposal.ProposalID
	proposal.Status = types2.StatusVotingPeriod
	keeper.SetProposal(ctx, proposal)

	require.NoError(t, keeper.AddVote(ctx, proposalID, sdk.ValAddress(valAccAddr1), types2.OptionYes))
	require.NoError(t, keeper.AddVote(ctx, proposalID, sdk.ValAddress(valAccAddr2), types2.OptionYes))
	require.NoError(t, keeper.AddVote(ctx, proposalID, sdk.ValAddress(valAccAddr3), types2.OptionYes))
