...
type SignatureVerificationGasConsumer = func(meter sdk.GasMeter, sig []byte, pubkey crypto.PubKey, params auth.Params) sdk.Result
...
func NewAnteHandler(ak auth.AccountKeeper, supplyKeeper types.SupplyKeeper, coinKeeper coin.Keeper, sigGasConsumer SignatureVerificationGasConsumer) sdk.AnteHandler {
	return func(
		...
		if !stdTx.Fee.Amount.IsZero() {
			res = DeductFees(supplyKeeper, coinKeeper, newCtx, signerAccs[0], stdTx.Fee.Amount)
			if !res.IsOK() {
				return newCtx, res, true
			}
...
func ValidateSigCount(stdTx auth.StdTx, params auth.Params) sdk.Result {
	...
	sigCount := 0
	for i := 0; i < len(stdSigs); i++ {
		sigCount += auth.CountSubKeys(stdSigs[i].PubKey)
...
func DeductFees(supplyKeeper types.SupplyKeeper, coinKeeper coin.Keeper, ctx sdk.Context, acc auth.Account, fees sdk.Coins) sdk.Result {
	...
	if fee.Denom != DefaultBondDenom {
		if feeCoin.Reserve.LT(fee.Amount) {
			return vtypes.ErrCoinReserveIsNotSufficient(DefaultCodespace, feeCoin.Reserve.String(), fee.Amount.String()).Result()
		}
...
// consensus.
func EnsureSufficientMempoolFees(ctx sdk.Context, stdFee auth.StdFee) sdk.Result {
