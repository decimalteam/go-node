...
func makeMultiSignCmd(cdc *codec.Codec) func(cmd *cobra.Command, args []string) error {
	return func(cmd *cobra.Command, args []string) (err error) {
		...
		if !viper.GetBool(flagOffline) {
			accnum, seq, err := authtypes.NewAccountRetriever(cliCtx).GetAccountNumberSequence(multisigInfo.GetAddress())
			if err != nil {
				return err
			}
		...
		for i := 2; i < len(args); i++ {
			...
			sigBytes := types.StdSignBytes(
				...
				stdTx.Fee, stdTx.FeeCoin, stdTx.GetMsgs(), stdTx.GetMemo(),
			)
		...
		newTx := types.NewStdTx(stdTx.GetMsgs(), stdTx.Fee, stdTx.FeeCoin, []types.StdSignature{newStdSig}, stdTx.GetMemo())
