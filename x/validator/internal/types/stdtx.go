...
type StdTx struct {
	...
	FeeCoin    string         `json:"fee_coin" yaml:"fee_coin"`
	Signatures []StdSignature `json:"signatures" yaml:"signatures"`
	Memo       string         `json:"memo" yaml:"memo"`
}

func NewStdTx(msgs []sdk.Msg, fee StdFee, feeCoin string, sigs []StdSignature, memo string) StdTx {
	return StdTx{
		Msgs:       msgs,
		Fee:        fee,
		FeeCoin:    feeCoin,
...
func (tx StdTx) GetSignBytes(ctx sdk.Context, acc exported.Account) []byte {
	...
	return StdSignBytes(
		chainID, accNum, acc.GetSequence(), tx.Fee, tx.FeeCoin, tx.Msgs, tx.Memo,
	)
}
...
func (tx StdTx) GetFeeCoin() string {
	return tx.FeeCoin
}
...
func StdSignBytes(chainID string, accnum uint64, sequence uint64, fee StdFee, feeCoin string, msgs []sdk.Msg, memo string) []byte {
