...
func NewTxBuilder(
	...
	simulateAndExecute bool, chainID, memo string, fees sdk.Coins, feeCoin string, gasPrices sdk.DecCoins,
) TxBuilder {
...
func NewTxBuilderFromCLI(input io.Reader) TxBuilder {
	...
	txbldr := TxBuilder{
		...
		feeCoin:            viper.GetString(flagFeeCoin),
...
func (bldr TxBuilder) Sign(name, passphrase string, msg StdSignMsg) ([]byte, error) {
	...
	return bldr.txEncoder(NewStdTx(msg.Msgs, msg.Fee, msg.FeeCoin, []StdSignature{sig}, msg.Memo))
}
...
func (bldr TxBuilder) BuildTxForSim(msgs []sdk.Msg) ([]byte, error) {
	...
	sigs := []StdSignature{{}}
	return bldr.txEncoder(NewStdTx(signMsg.Msgs, signMsg.Fee, signMsg.FeeCoin, sigs, signMsg.Memo))
}
...
func (bldr TxBuilder) SignStdTx(name, passphrase string, stdTx StdTx, appendSig bool) (signedStdTx StdTx, err error) {
	...
	signedStdTx = NewStdTx(stdTx.GetMsgs(), stdTx.Fee, stdTx.FeeCoin, sigs, stdTx.GetMemo())
	return
}
