...
func ValidateBasic(hi HistoricalInfo) error {
	if len(hi.ValSet) == 0 {
		return sdkerrors.Wrap(ErrInvalidHistoricalInfo(DefaultCodespace), "validator set is empty")
	}
	if !sort.IsSorted(Validators(hi.ValSet)) {
		return sdkerrors.Wrap(ErrInvalidHistoricalInfo(DefaultCodespace), "validator set is not sorted by address")
	}
	return nil
}
