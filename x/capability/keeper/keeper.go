...
func (k *Keeper) InitializeAndSeal(ctx sdk.Context) {
	...
	prefixStore := prefix.NewStore(ctx.KVStore(k.storeKey), types2.KeyPrefixIndexCapability)
	...
	for ; iterator.Valid(); iterator.Next() {
		index := types2.IndexFromKey(iterator.Key())
...
func (k Keeper) InitializeIndex(ctx sdk.Context, index uint64) error {
	...
	store.Set(types2.KeyIndex, types2.IndexToKey(index))
	return nil
}
...
func (k Keeper) GetLatestIndex(ctx sdk.Context) uint64 {
	store := ctx.KVStore(k.storeKey)
	return types2.IndexFromKey(store.Get(types2.KeyIndex))
}
...
func (k Keeper) SetOwners(ctx sdk.Context, index uint64, owners types.CapabilityOwners) {
	prefixStore := prefix.NewStore(ctx.KVStore(k.storeKey), types2.KeyPrefixIndexCapability)
	indexKey := types2.IndexToKey(index)
...
func (k Keeper) InitializeCapability(ctx sdk.Context, index uint64, owners types.CapabilityOwners) {
	...
	cap := types2.NewCapability(index)
	for _, owner := range owners.Owners {
		// Set the forward mapping between the module and capability tuple and the
		// capability name in the memKVStore
		memStore.Set(types2.FwdCapabilityKey(owner.Module, cap), []byte(owner.Name))

		// Set the reverse mapping between the module and capability name and the
		// index in the in-memory store. Since marshalling and unmarshalling into a store
		// will change memory address of capability, we simply store index as value here
		// and retrieve the in-memory pointer to the capability from our map
		memStore.Set(types2.RevCapabilityKey(owner.Module, owner.Name), sdk.Uint64ToBigEndian(index))
...
func (sk ScopedKeeper) NewCapability(ctx sdk.Context, name string) (*types.Capability, error) {
	...
	index := types2.IndexFromKey(store.Get(types2.KeyIndex))
	cap := types2.NewCapability(index)
	...
	store.Set(types2.KeyIndex, types2.IndexToKey(index+1))

	memStore := ctx.KVStore(sk.memKey)

	// Set the forward mapping between the module and capability tuple and the
	// capability name in the memKVStore
	memStore.Set(types2.FwdCapabilityKey(sk.module, cap), []byte(name))

	// Set the reverse mapping between the module and capability name and the
	// index in the in-memory store. Since marshalling and unmarshalling into a store
	// will change memory address of capability, we simply store index as value here
	// and retrieve the in-memory pointer to the capability from our map
	memStore.Set(types2.RevCapabilityKey(sk.module, name), sdk.Uint64ToBigEndian(index))
...
func (sk ScopedKeeper) ClaimCapability(ctx sdk.Context, cap *types.Capability, name string) error {
	...
	// will change memory address of capability, we simply store index as value here
	// and retrieve the in-memory pointer to the capability from our map
	memStore.Set(types2.RevCapabilityKey(sk.module, name), sdk.Uint64ToBigEndian(cap.GetIndex()))
...
func (sk ScopedKeeper) ReleaseCapability(ctx sdk.Context, cap *types.Capability) error {
	...
	// capability name in the memKVStore
	memStore.Delete(types2.FwdCapabilityKey(sk.module, cap))

	// Delete the reverse mapping between the module and capability name and the
	// index in the in-memory store.
	memStore.Delete(types2.RevCapabilityKey(sk.module, name))

	// remove owner
	capOwners := sk.getOwners(ctx, cap)
	capOwners.Remove(types2.NewOwner(sk.module, name))

	prefixStore := prefix.NewStore(ctx.KVStore(sk.storeKey), types2.KeyPrefixIndexCapability)
	indexKey := types2.IndexToKey(cap.GetIndex())
...
func (sk ScopedKeeper) GetCapability(ctx sdk.Context, name string) (*types.Capability, bool) {
	...
	key := types2.RevCapabilityKey(sk.module, name)
...
func (sk ScopedKeeper) GetCapabilityName(ctx sdk.Context, cap *types.Capability) string {
	...
	return string(memStore.Get(types2.FwdCapabilityKey(sk.module, cap)))
}
...
func (sk ScopedKeeper) addOwner(ctx sdk.Context, cap *types.Capability, name string) error {
	...
	if err := capOwners.Set(types2.NewOwner(sk.module, name)); err != nil {
		return err
	}
...
func (sk ScopedKeeper) getOwners(ctx sdk.Context, cap *types.Capability) *types.CapabilityOwners {
	...
	if len(bz) == 0 {
		return types2.NewCapabilityOwners()
	}
