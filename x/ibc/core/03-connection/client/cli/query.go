...
import (
	...
	"bitbucket.org/decimalteam/go-node/x/ibc/core/03-connection/client/utils"
...
func GetCmdQueryConnections() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf("%s query %s %s connections", version.AppName, host.ModuleName, clienttypes.SubModuleName),
		Args:    cobra.NoArgs,
...
func GetCmdQueryConnection() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf("%s query %s %s end [connection-id]", version.AppName, host.ModuleName, clienttypes.SubModuleName),
...
func GetCmdQueryClientConnections() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf("%s query  %s %s path [client-id]", version.AppName, host.ModuleName, clienttypes.SubModuleName),
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientQueryContext(cmd)
			if err != nil {
