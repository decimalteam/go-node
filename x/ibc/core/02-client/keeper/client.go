...
func (k Keeper) UpdateClient(ctx sdk.Context, clientID string, header exported.Header) error {
	...
	if !found {
		return sdkerrors.Wrapf(types2.ErrClientNotFound, "cannot update client with ID %s", clientID)
	}
	...
	if clientState.IsFrozen() && clientState.GetFrozenHeight().LTE(header.GetHeight()) {
		return sdkerrors.Wrapf(types2.ErrClientFrozen, "cannot update client with ID %s", clientID)
	}
	...
	if header != nil && clientID != exported.Localhost {
		...
		consensusHeight = types2.GetSelfHeight(ctx)
	}
	...
	if header != nil {
		...
		headerStr = hex.EncodeToString(types2.MustMarshalHeader(k.cdc, header))
	...
	ctx.EventManager().EmitEvent(
		sdk.NewEvent(
			types2.EventTypeUpdateClient,
			sdk.NewAttribute(types2.AttributeKeyClientID, clientID),
			sdk.NewAttribute(types2.AttributeKeyClientType, clientState.ClientType()),
			sdk.NewAttribute(types2.AttributeKeyConsensusHeight, consensusHeight.String()),
			sdk.NewAttribute(types2.AttributeKeyHeader, headerStr),
		),
	)
...
func (k Keeper) UpgradeClient(ctx sdk.Context, clientID string, upgradedClient exported.ClientState, upgradedConsState exported.ConsensusState,
	...
	ctx.EventManager().EmitEvent(
		sdk.NewEvent(
			types2.EventTypeUpgradeClient,
			sdk.NewAttribute(types2.AttributeKeyClientID, clientID),
			sdk.NewAttribute(types2.AttributeKeyClientType, updatedClientState.ClientType()),
			sdk.NewAttribute(types2.AttributeKeyConsensusHeight, updatedClientState.GetLatestHeight().String()),
		),
	)
...
func (k Keeper) CheckMisbehaviourAndUpdateState(ctx sdk.Context, misbehaviour exported.Misbehaviour) error {
	...
	if !found {
		return sdkerrors.Wrapf(types2.ErrClientNotFound, "cannot check misbehaviour for client with ID %s", misbehaviour.GetClientID())
	}

	if clientState.IsFrozen() && clientState.GetFrozenHeight().LTE(misbehaviour.GetHeight()) {
		return sdkerrors.Wrapf(types2.ErrInvalidMisbehaviour, "client is already frozen at height ≤ misbehaviour height (%s ≤ %s)", clientState.GetFrozenHeight(), misbehaviour.GetHeight())
	}
